...
class CTWalks(torch.nn.Module):
    def __init__(self, n_feat, e_feat, walk_mutual=False, walk_linear_out=False, pos_enc='saw', pos_dim=0, num_layers=3,
                 ...
                 verbosity=1, get_checkpoint_path=None,community_labels=None):

        super(CTWalks, self).__init__()
        ...
        self.position_encoder = PositionEncoder(enc_dim=self.pos_dim, num_layers=self.num_layers,
                                                ...
                                                verbosity=verbosity, logger=self.logger, enc=self.pos_enc, community_labels=community_labels)
...
class PositionEncoder(nn.Module):
    def __init__(self, num_layers, enc='saw', enc_dim=2, ngh_finder=None, verbosity=1, cpu_cores=1, logger=None, community_labels=None):
    ...
    def get_community_labels(self, node_indices):
        """
        获取 root 节点的社区标签.
        ...
        if self.community_labels is None:
            raise ValueError("Community labels are not set. Please pass them during initialization.")
        ...
        # 使用列表索引获取社区标签
        labels = [self.community_labels[node] if node < len(self.community_labels) else -1 for node in node_indices]
        return labels
    ...
    def forward(self, node_record, t_record):
        ...
        # 获取每个 walk 的 root 节点
        batch_size, walk_length = node_record.shape
        root_nodes = node_record[:, 0]  # 假设 root 节点是 walk 的第一个节点
        root_labels = self.get_community_labels(root_nodes)  # 调用 get_community_labels
        root_labels = torch.tensor(root_labels).to(device).unsqueeze(-1)  # 转为张量，增加维度

        # 扩展社区标签为与匿名化结构一致的形状
        expanded_labels = root_labels.repeat(1, walk_length).unsqueeze(-1)  # [batch, walk_length, 1]

        # 将社区标签添加到匿名化结构中
        encodings = torch.cat([encodings, expanded_labels], dim=-1)

        walk_encodings = None
        common_nodes = (((encodings.sum(-1) > 0).sum(-1) == 2).sum().float() / (
                    encodings.shape[0] * encodings.shape[1])).item()
